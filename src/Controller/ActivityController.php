...
use App\Forms\ActivityTypeForm;
...
class ActivityController extends AbstractController
{
    ...
    public function deleteAction(Request $request, Activity $id, LoggerInterface $logger)
    {
        ...
        $this->denyAccessUnlessGranted('ROLE_ADMIN', null, 'Unable to access this page!');
        $em = $this->getDoctrine()->getManager();
        $ticket = $em->getRepository(BuyTickets::class)->findOneBy(['activity' => $id]);
        if (null !== $ticket) {
            $this->addFlash('error', 'La actividad no puede borrarse, porque se han creado tickets de la misma.');
